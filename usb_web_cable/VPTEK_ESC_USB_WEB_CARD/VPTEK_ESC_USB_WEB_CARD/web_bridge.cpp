...
constexpr char AP_SSID[] = "VPTEK-ESC_002";
...
bool readMemory( uint16_t address, uint16_t length, uint8_t *destination)
{
    ...
    const FourWayResult result = runFourWay( cmd_DeviceRead, address, &readLength, 1);
...
void applyKnownArguments()
{
    ...
    argToByte("use_sine_start", eepromData[19]);
    argToByte("comp_pwm", eepromData[20]);
...
void handleSave()
{
    ...
    if (!readMemory( eepromAddress, EEPROM_SIZE_BYTES, first)) {
        sendError( 503,"Không đọc được EEPROM trước khi lưu");
        return;
    }

    delay(180);

    if (!readMemory(eepromAddress, EEPROM_SIZE_BYTES, second) || memcmp(first, second, EEPROM_SIZE_BYTES) != 0 || !validEepromHeader(second, EEPROM_SIZE_BYTES)) {
...
<title>VPTEK ESC Control Center</title>
<style>
...
<body>
<section id="connectionChooser" class="connectionChooser">
  <div class="connectionChooserPanel">
    <div class="connectionChooserBrand">
      <span class="connectionChooserLogo">VP</span>
      <span>
        <span class="connectionChooserTitle">Choose Connection</span>
        <span class="connectionChooserSubtitle">Select the device you are using</span>
      </span>
    </div>

    <div class="connectionChooserGrid">
      <button class="connectionChoice" onclick="choosePCConnection()">
        <span class="connectionChoiceIcon" aria-hidden="true">PC/Laptop</span>
        <span class="connectionChoiceTitle">PC Connect</span>
        <span class="connectionChoiceText">
          Continue with the configurator in this browser.
        </span>
      </button>

      <button class="connectionChoice" onclick="chooseMobileConnection()">
        <span class="connectionChoiceIcon" aria-hidden="true">Mobile(android/IOS)</span>
        <span class="connectionChoiceTitle">Mobile Connect</span>
        <span class="connectionChoiceText">
          Open the configurator in Chrome for reliable firmware installation.
        </span>
      </button>
    </div>

    <div id="mobileBrowserFallback" class="mobileBrowserFallback" hidden>
      Chrome could not be opened automatically. Close this window, open
      Chrome or Safari, then enter:
      <span class="mobileBrowserAddress">http://192.168.4.1/?external=1</span>
    </div>
  </div>
</section>
...
<div class="top">
  <div class="brand">
    <div class="brandMark">VP</div>
    <div>
      <div class="eyebrow">VPTEK</div>
      <h1>VPTEK ESC Control Center</h1>
...
<div class="tabs">
  ...
  <button class="tab" data-page="motor">Motor Control</button>
...
<div class="checkboxRow"><input data-field="use_hall_sensors" type="checkbox"><label>Use Hall Sensors</label></div>
<div class="checkboxRow"><input data-field="disable_stick_calibration" type="checkbox"><label>Disable Stick Calibration</label></div>
<div class="checkboxRow"><input data-field="use_sine_start" type="checkbox"><label>Sinusoidal Startup</label></div>
</div>
...
<div class="defaultFirmware">
  <div>
    <strong>Recommended VPTEK Firmware</strong>
                      <span>Firmware supported by manufacturer VPTEK</span>
  </div>
  <button
    class="primary"
    id="defaultFlashButton"
    onclick="flashDefaultFirmware()"
  >Install VPTEK Firmware</button>
</div>
...
<div class="cardKicker">Import or export ESC settings</div>
...
<section id="motor" class="page">
<div class="card motorPanel">
<h3>Motor Control</h3>
<div class="disabledTag">Unavailable in Web Mode</div>
<div class="checkboxRow"><input type="checkbox" disabled><label>MSP Motor Control</label></div>
<div class="motorReadout"><label>All Motors</label><input type="range" min="1000" max="2000" value="1000" disabled><span>1000</span></div>
<div class="motorReadout"><label>Motor 1</label><input type="range" min="1000" max="2000" value="1000" disabled><span>1000</span></div>
<div class="motorReadout"><label>Motor 2</label><input type="range" min="1000" max="2000" value="1000" disabled><span>1000</span></div>
<div class="motorReadout"><label>Motor 3</label><input type="range" min="1000" max="2000" value="1000" disabled><span>1000</span></div>
<div class="motorReadout"><label>Motor 4</label><input type="range" min="1000" max="2000" value="1000" disabled><span>1000</span></div>
</div>
</section>

</div>

<div class="footer">VPTEK ESC · Local Device Interface</div>

<div id="toast" class="toast"></div>

<script>
function hideConnectionChooser(){
  const chooser=document.getElementById('connectionChooser');
  if(chooser)chooser.hidden=true;
}

function choosePCConnection(){
  try{
    history.replaceState(null,'','/?pc=1');
  }catch(error){}
  hideConnectionChooser();
}

function showMobileBrowserFallback(){
  const fallback=document.getElementById('mobileBrowserFallback');
  if(fallback)fallback.hidden=false;
}

function chooseMobileConnection(){
  ...
  if(isAndroid){
    window.location.href=
      'intent://192.168.4.1/?external=1'+
      '#Intent;scheme=http;package=com.android.chrome;end';
    setTimeout(showMobileBrowserFallback,1400);
    return;
  }

  if(isIOS){
    window.location.href='googlechrome://192.168.4.1/?external=1';
    setTimeout(showMobileBrowserFallback,1400);
    return;
  }

  showMobileBrowserFallback();
}

try{
  const connectionMode=
    new URLSearchParams(window.location.search);
  if(connectionMode.has('pc')||connectionMode.has('external')){
    hideConnectionChooser();
  }
}catch(error){}
...
function uiMessage(message){
  ...
  const exact={
    ...
    'Không nhận được bootloader ESC':'ESC bootloader not detected',
...
function downloadConfig(){
  ...
  link.download=`VPTEK_ESC_FW_${state.firmwareMajor}_${state.firmwareMinor}_Config.json`;
  link.click();
...
async function flashDefaultFirmware(){
  ...
  if(!confirm('Install the recommended VPTEK firmware?'))return;

  setBusy(true);
  resetLocalFinalization();
  showFlashStage('none');
  $('#flashProgress').value=0;
  $('#flashText').textContent='Preparing VPTEK firmware';
...
void setupRoutes()
{
    ...
    server.onNotFound([]() {
        if (server.uri().startsWith("/api/")) {
            sendError(404, "Không tìm thấy API");
            return;
        }
        redirectToPortal();
    });
...
{
    if (data == nullptr || length != EEPROM_SIZE_BYTES) {
        message = "Invalid EEPROM buffer";
        return false;
    }

    if (webBridgeLocksEsc()) {
        message = "ESC interface is busy";
        return false;
    }
    ...
    if (!connectEscInternal() || !readEepromInternal()) {
        message = lastMessage;
        return false;
    }

    if (!validEepromHeader(eepromData, EEPROM_SIZE_BYTES) ||
        eepromData[0] != 1) {
        message = "ESC firmware/settings are not ready";
        return false;
    }

    memcpy(data, eepromData, EEPROM_SIZE_BYTES);
    message = "ESC settings loaded";
    return true;
}
...
bool cardWriteEscSettings(const uint8_t *data, size_t length, String &message)
{
    ...
    // Re-enter the bootloader so Card works even after USB/Web reset the ESC.
    escConnected = false;
    eepromLoaded = false;
    firmwareMissing = false;
    eepromRawAvailable = false;
    ...
    memcpy(eepromData, data, EEPROM_SIZE_BYTES);
    eepromData[0] = 1;
    ...
    uint8_t verify[EEPROM_SIZE_BYTES] = {0};
    if (!readMemory(eepromAddress, EEPROM_SIZE_BYTES, verify)) {
        message = "EEPROM verify read failed";
        return false;
    }

    for (size_t i = 0; i < EEPROM_SIZE_BYTES; ++i) {
        // The bootloader may update byte 2.
        if (i != 2 && verify[i] != eepromData[i]) {
            message = "EEPROM verify failed at " + String(i);
            return false;
        }
    }
...
bool cardStartBuiltInFirmware(String &message)
{
    ...
    // Always reconnect and read the physical EEPROM before changing its
    // safety byte. This also allows Card mode to recover an interrupted flash.
    escConnected = false;
    eepromLoaded = false;
    firmwareMissing = false;
    eepromRawAvailable = false;
    ...
    const bool settingsOk = readEepromInternal();
    if (!settingsOk && !(firmwareMissing && eepromRawAvailable)) {
        message = lastMessage;
        return false;
    }
    ...
    // Card flashing does not need an HTTP response delay. Stop the AP first,
    // then start the same real block writer used by the Web firmware page.
    stopWebRadio();
    beginFlash();

    if (flashState == FlashState::Error) {
        message = flashError;
        return false;
    }
...
{
    CardFirmwareStatus status = {
        CardFirmwareStage::Idle,
        firmwareWritten,
        firmwareSize,
        0,
        lastMessage
    ...
    if (firmwareSize > 0) {
        const uint32_t calculated =
            (firmwareWritten * 100UL) / firmwareSize;
        status.percent = static_cast<uint8_t>(
            calculated > 100 ? 100 : calculated);
    }

    switch (flashState) {
        case FlashState::Ready:
        case FlashState::Pending:
            status.stage = CardFirmwareStage::Preparing;
            break;
        case FlashState::SafetyOff:
            status.stage = CardFirmwareStage::Safety;
            break;
        case FlashState::Writing:
            status.stage = CardFirmwareStage::Writing;
            break;
        case FlashState::SafetyOn:
            status.stage = CardFirmwareStage::Finalizing;
            break;
        case FlashState::ResetEsc:
            status.stage = CardFirmwareStage::Resetting;
            break;
        case FlashState::Done:
            status.stage = CardFirmwareStage::Done;
            status.percent = 100;
            break;
        case FlashState::Error:
            status.stage = CardFirmwareStage::Error;
            status.message = flashError;
            break;
        default:
            status.stage = CardFirmwareStage::Idle;
            break;
    }
...
void webBridgeLoop()
{
     ...
     */
    if (!webRadioRunning && !webFlashResumePending && !webFlashPending && (flashState == FlashState::Done || flashState == FlashState::Error)) {
