...
uint16_t Check_4Way(uint8_t buf[]) {
  ...
  /*
   * The Web path initializes GPIO21 before issuing a 4-Way request, while
   * a USB App Tool starts directly with 4-Way commands.  Initialize the
   * half-duplex serial link here as well so both entry points behave alike.
   * InterfaceExit deliberately remains the one command that can leave it
   * deinitialized.
   */
  if (!Enable4Way && cmd != cmd_InterfaceExit) {
    InitSerialOutput();
    delay(30);
  }
  ...
  if (cmd == cmd_DeviceInitFlash) {
    ...
    if (param == 0x00) {  // ESC Count
      ...
      /*
       * A freshly powered ESC can ignore the first bootloader request.
       * Retrying only this harmless discovery request prevents the App Tool
       * from reporting "BAD OR NO ACK FROM IF" while never retrying writes,
       * erases, or other state-changing commands.
       */
      for (uint8_t attempt = 0; attempt < 3; ++attempt) {
        memset(RX_Buf, 0, sizeof(RX_Buf));
        // DeviceInitFlash already contains its own CRC.
        SendESC(BootInit, Init_Size, false);
        delay(50);
        // BootMsg + signature + boot version + pages + brSUCCESS (0x30).
        RX_Size = GetESC(RX_Buf, 200, sizeof(RX_Buf));
    ...
    if ((param == 0x00)/*&& (addr_high == 0x00)*/) {  // ESC Count
      if (Enable4Way) {
        ...
        uint8_t RX_Buf[5] = {0};
        RX_Size = GetESC(RX_Buf, 50, sizeof(RX_Buf));
      }
      else {
        ack_out = ACK_D_GENERAL_ERROR;
    ...
    if (RX_Buf[0] == brSUCCESS) {
      ...
      RX_Size = GetESC(RX_Buf, 500, sizeof(RX_Buf));
#ifdef _DEBUG_
    ...
    RX_Size = SendESC(ESC_data, Data_Size);
    delay(50);
    RX_Size = GetESC(RX_Buf, 100, sizeof(RX_Buf));
#ifdef _DEBUG_
    Serial.print("ESC Received: ");
    Serial.print(RX_Size);
    Serial.print(" Bytes: ");
